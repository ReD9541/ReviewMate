...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['movie_id']) && is_numeric($_POST['movie_id'])) {
    ...
    if ($check_movie_result->num_rows > 0) {
        ...
        if ($check_watched_result->num_rows > 0) {
            echo "<script>alert('You\'ve already marked the movie as watched.');window.location.href='/ReviewMate/movie/movie_details.php?movie_id=" . $movie_id . "';</script>";
        } else {
            ...
            if ($stmt->execute()) {
                header("Location: /ReviewMate/movie/movie_details.php?movie_id=" . $movie_id);
                exit();
            } else {
                echo "<script>alert('Error marking as watched.');window.location.href='/ReviewMate/movie/movie_details.php?movie_id=" . $movie_id . "';</script>";
            }
